...
수업의 Transformers **4.57.6**에서 직접 작성한 추론 함수와 비교할 수 있도록 전처리는 `use_fast=False`, Attention 계산 방식은 `eager`로 맞췄습니다. Python 파일의 `create_classifier()`도 같은 설정을 사용합니다.
...
01번에는 Pipeline 예제가 함께 들어 있습니다. 이어지는 두 빈칸에서는 Pipeline이 묶어 준 처리 중 전처리·추론과 Top-k 정리를 직접 작성합니다. Pipeline 예제 실행만으로 빈칸이 자동 완성되지는 않습니다.
...
3. Pipeline 결과와 직접 작성한 01번 실습 함수의 결과를 같은 이미지에서 비교합니다. 모델·전처리·입력·장치가 같았는지 먼저 확인합니다.
...
시연 후에는 01·02번의 AI 코딩 빈칸을 채우고 [기본 실습 안내](README.md)의 수동 실행으로 이어갑니다. 이 자동 추론 시연과 00·01·02번 학습 과정은 결과 폴더와 세션을 나누어 사용합니다.
